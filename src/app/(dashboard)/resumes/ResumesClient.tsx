...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`text-xs px-2 py-1 rounded-full font-medium ${styles[status] ?? "bg-slate-100 text-slate-600"}`}
    >
      {status}
    </span>
  );
...
export default function ResumesClient({ initialResumes }: { initialResumes: ResumeItem[] }) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {resumes.map((resume) => (
        <div key={resume.id} className="relative group">
          ...
          {/* Delete button — only visible on DRAFT resumes */}
          {resume.status === "DRAFT" && (
            <button
