...
export default function SkillsManager({ initialSkills }: SkillsManagerProps) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setSkills((prev) => [
        ...prev,
        { id: created._id, name: created.name, category: created.category, level: created.level },
      ]);
      setName("");
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch(`/api/career/skills/${id}`, { method: "DELETE" });
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm">
            Manage your skills to power resume generation.
          </p>
        </div>
        {!showForm && (
          <button
      ...
      {skills.length === 0 && !showForm ? (
        <div className="border-2 border-dashed border-slate-200 rounded-xl p-12 text-center">
          ...
          <p className="text-slate-400 text-sm mb-6">
            Add your technical skills, languages, and tools.
          </p>
          <button
            onClick={() => setShowForm(true)}
              ...
              <div key={cat} className="bg-white border border-slate-200 rounded-xl p-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {catSkills.map((skill) => (
                    <div
                      key={skill.id}
                      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium border ${CATEGORY_COLORS[cat]}`}
                    >
                      <span>{skill.name}</span>
                      <span className="opacity-60 text-xs">· {LEVEL_LABELS[skill.level]}</span>
                      <button
                        onClick={() => handleDelete(skill.id)}
                        className="ml-1 opacity-60 hover:opacity-100 text-xs leading-none transition-opacity"
                        title="Remove skill"
                      >
                        ✕
                      </button>
