...
export default function ExperienceManager({ initialExperiences }: ExperienceManagerProps) {
  ...
  const handleAdd = async (form: FormState) => {
    ...
    try {
      ...
      setExperiences((prev) => [mapped, ...prev]);
      setShowForm(false);
    } catch {
  ...
  const handleEdit = async (id: string, form: FormState) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/career/experience/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          company: form.company,
      ...
      setExperiences((prev) => prev.map((e) => (e.id === id ? mapped : e)));
      setEditingId(null);
    } catch {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/career/experience/${id}`, { method: "DELETE" });
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm">
            Add your work history to power AI resume generation.
          </p>
        </div>
        {!showForm && (
          <button
      ...
      {experiences.length === 0 && !showForm ? (
        <div className="border-2 border-dashed border-slate-200 rounded-xl p-12 text-center">
          ...
          <p className="text-slate-400 text-sm mb-6">
            Add your experience to help AI tailor your resumes.
          </p>
          <button
          ...
          {experiences.map((exp) =>
            editingId === exp.id ? (
              <ExperienceForm
                ...
                onSave={(form) => handleEdit(exp.id, form)}
