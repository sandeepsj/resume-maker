...
function EducationForm({ initial, onSave, onCancel, saving }: EducationFormProps) {
  ...
  const addActivity = () => set("activities", [...form.activities, ""]);
...
export default function EducationManager({ initialEducations }: EducationManagerProps) {
  ...
  const handleAdd = async (form: FormState) => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/career/education", {
        method: "POST",
  ...
  const handleEdit = async (id: string, form: FormState) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/career/education/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          institution: form.institution,
      ...
      const mapped: EducationData = {
        ...
        honors: updated.honors ?? null,
        activities: updated.activities,
      };
      setEducations((prev) => prev.map((e) => (e.id === id ? mapped : e)));
      setEditingId(null);
    } catch {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/career/education/${id}`, { method: "DELETE" });
      ...
      {educations.length === 0 && !showForm ? (
        <div className="border-2 border-dashed border-slate-200 rounded-xl p-12 text-center">
          ...
          <p className="text-slate-400 text-sm mb-6">
            Add your educational background for AI-tailored resumes.
          </p>
          <button
        ...
        <div className="space-y-4">
          {educations.map((edu) =>
            editingId === edu.id ? (
              <EducationForm
                key={edu.id}
                initial={educationToForm(edu)}
                onSave={(form) => handleEdit(edu.id, form)}
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-2 ml-4 shrink-0">
                    <button
                      ...
                      className="text-sm border border-slate-300 hover:bg-slate-50 text-slate-700 rounded-lg px-3 py-1.5 transition-colors"
                    >
                      Edit
                    </button>
                    <button
                      onClick={() => handleDelete(edu.id)}
                      className="text-sm bg-red-500 hover:bg-red-600 text-white rounded-lg px-3 py-1.5 transition-colors"
                    >
                      Delete
                    </button>
                  </div>
